...
data <- subset(data, Country %in% g19.countries)
# data <- subset(data, Country %in% south.america)
...
```
We can deduce that the percent of Forest Coverage accross Mercosur's Countries
decreased from ~53% to 46% 1990 and 2005.
...
ggplot(argentina) + 
  ...
  ggtitle("Forest Coverage (%) Evolution in Argentina by a period of 5 Years\n 
          compared with the average of Mercosur") + 
  ylab("Forest Coverage (%)") 
```
...
ggplot(top.countries.2005[1:20,]) + 
  ...
  ggtitle("Forest Coverage (%) in Mercosur Countries in 2005.")
```

```{r}
Country <- as.character(unique(data$Country))
Evolution <- subset(data, Year == 2005)$Forest.Coverage - subset(data, Year == 1990)$Forest.Coverage

countries.evolution <- data.frame(Country, Evolution)
countries.evolution <- arrange(countries.evolution,Evolution,decreasing = T)
countries.evolution$Country <- factor(countries.evolution$Country,
                                      levels = countries.evolution$Country) 

ggplot(countries.evolution[1:20,]) + 
  geom_bar(aes(x=Country,y=Evolution), fill = "blue4", stat = "identity") +
...
sPDF <- joinCountryData2Map( countries.evolution, joinCode = "NAME", 
                             nameJoinColumn = "Country")
mapCountryData( sPDF, nameColumnToPlot="Evolution",
